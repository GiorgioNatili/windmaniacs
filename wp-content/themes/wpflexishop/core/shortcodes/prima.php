...
function prima_products_shortcodes($atts, $content=null){
	...
	if ( $multislides->have_posts() ) :
		...
		while ($multislides->have_posts()) : $multislides->the_post();
			...
			$output .= '<li style="width:'.$box_width.'px;height:300px;margin-bottom:10px;">';
			$output .= '<div class="prima-multislide-box" style="width:'.$image_width.'px;height:300px;'.($image_bg ? 'background:'.$image_bg.';' : '').'">';
			...
			$output .= '<div class="price">'.$post_price.'</div>';
	        $output .= '<h3><a href="'.$post_url.'" rel="bookmark">'.$post_name.'</a></h3>';
	        $output .= '<div class="excerpt">'.$post_excertpt.'</div>';
	        $output .= '<a href="'.$post_url.'" class="read-more">Visualizza</a>';
			$output .= '</div>';
