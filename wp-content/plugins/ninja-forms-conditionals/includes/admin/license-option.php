...
function ninja_forms_conditionals_modify_license_label(){
	...
	for ($x=0; $x < count( $ninja_forms_tabs_metaboxes['ninja-forms-settings']['license_settings']['license_settings']['settings'] ); $x++) { 
		if( $ninja_forms_tabs_metaboxes['ninja-forms-settings']['license_settings']['license_settings']['settings'][$x]['name'] == 'conditionals_license' ){
			$plugin_settings = get_option( 'ninja_forms_settings' );
			if( !isset( $plugin_settings['conditionals_license_status'] ) OR $plugin_settings['conditionals_license_status'] == 'invalid' ){
...
function ninja_forms_conditionals_activate_license( $data ){
	$plugin_settings = get_option( 'ninja_forms_settings' );
	if( isset( $plugin_settings['conditionals_license_status'] ) ){
		$status = $plugin_settings['conditionals_license_status'];
	}else{
		$status = 'invalid';
	}

	if( isset( $plugin_settings['conditionals_license'] ) ){
		$old_license = $plugin_settings['conditionals_license'];
	}else{
		$old_license = '';
	}

	if( $old_license == '' OR ( $old_license != $data['conditionals_license'] ) OR $status == 'invalid' ){
 		// retrieve the license from the database
		$license = $data['conditionals_license'];

		// data to send in our API request
		$api_params = array( 
			'edd_action'=> 'activate_license', 
			'license' 	=> $license, 
			'item_name' => urlencode( NINJA_FORMS_CON_EDD_SL_ITEM_NAME ) // the name of our product in EDD
		);
 
		// Call the custom API.
		$response = wp_remote_get( add_query_arg( $api_params, NINJA_FORMS_CON_EDD_SL_STORE_URL ) );
		...
		// $license_data->license will be either "valid" or "invalid"
 		$plugin_settings = get_option( 'ninja_forms_settings' );
 		$plugin_settings['conditionals_license_status'] = $license_data->license;
