...
function ninja_forms_save_browse_uploads(){
	global $ninja_forms_admin_update_message;
	if(isset($_POST['bulk_action']) AND $_POST['bulk_action'] == 'delete'){
		if(isset($_POST['ninja_forms_uploads'])){
			if(is_array($_POST['ninja_forms_uploads']) AND !empty($_POST['ninja_forms_uploads'])){
				foreach($_POST['ninja_forms_uploads'] as $upload){
					ninja_forms_delete_upload($upload);
				}
				$ninja_forms_admin_update_message = count( $_POST['ninja_forms_uploads'] ).' ';
				if( count( $_POST['ninja_forms_uploads'] ) > 1 ){
					$ninja_forms_admin_update_message .= __( 'Uploads Deleted', 'ninja-forms' );
				}else{
					$ninja_forms_admin_update_message .= __( 'Upload Deleted', 'ninja-forms' );
				}
			}
		}
	}
