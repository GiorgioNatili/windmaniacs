<?php
//Because we need the form_id of the current form in our field registration, this field is registered twice.
//Once for the backend, upon 'init'
add_action( 'init', 'ninja_forms_register_field_post_terms', 11 );
//And once just before our form is displayed on the frontend.
if( !is_admin() ){
	add_action( 'ninja_forms_before_form_display', 'ninja_forms_register_field_post_terms' );
}
function ninja_forms_register_field_post_terms( $form_id = '' ){
	...
	if( $form_id != '' AND $form_id != 'new' AND $form_id != 'all' ){
		...
		if( isset( $form_data['post_tax'] ) ){
			$post_tax = $form_data['post_tax'];
		}else{
			$post_tax = '';
		}
		
		if( is_array( $post_tax ) AND !empty( $post_tax) ){
		...
		<div id="taxonomy_<?php echo $field_id;?>" class="termdiv">
				...
				<?php
					if(is_array( $all_terms ) AND !empty( $all_terms ) ){
							...
							<li id="<?php echo $field_id;?>_<?php echo $term->term_id;?>_li" class="popular-term">
								...
								<?php
								$child_terms = get_categories( array( 'taxonomy' => $post_tax, 'child_of' => $term->term_id, 'hide_empty' => false ) );
			...
			<div id="term-adder" class="wp-hidden-children">
				...
				<p id="<?php echo $field_id;?>_add" class="term-add wp-hidden-child" style="display:none;">
					<input type="hidden" id="new_<?php echo $field_id;?>_default" value="<?php _e( 'New Term Name', 'ninja-forms' );?>">
					<input type="text" name="" id="new_<?php echo $field_id;?>_label" class="new-term-label" value="<?php _e( 'New Term Name', 'ninja-forms' );?>">
					<br />
