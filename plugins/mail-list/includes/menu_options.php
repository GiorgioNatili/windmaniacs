...
function mail_list_options() {
    ...
    $label6 = get_option('mail_list_label6');
    $terms_link = get_option('mail_list_terms_link');
    
    // Save options if user has posted some information
    if(isset($_POST['label1']) or isset($_POST['label2']) or isset($_POST['label3']) or isset($_POST['label4']) or isset($_POST['label6']) or isset($_POST['termsanchor']) or isset($_POST['termslink']) or isset($_POST['formcolor'])){        
        $label1=$_POST['label1'];$label2=$_POST['label2'];$label3=$_POST['label3'];$label4=$_POST['label4'];$label6=$_POST['label6'];$terms_anchor=$_POST['termsanchor'];$terms_link=$_POST['termslink'];$form_color=$_POST['formcolor'];
        //regexp check
        $error=0;
	    if(preg_match('/[^a-z_, .?\-0-9]/i',$label1)){$errors[]='Only letters and characters [. , ?] are allowed in "Welcome Message"';}
	    if(preg_match('/[^a-z_, .?\-0-9]/i',$label6)){$errors[]='Only letters and characters [. , ?] are allowed in "Contact text"';}
        ...
        if(count($errors)==0){
			// Save into database
			update_option('mail_list_label1',$label1);update_option('mail_list_label6',$label6);update_option('mail_list_label2',$label2);update_option('mail_list_label3',$label3);update_option('mail_list_label4',$label4);update_option('mail_list_terms_anchor',$terms_anchor);update_option('mail_list_terms_link',$terms_link);update_option('mail_list_form_color',$form_color);
	...
	<div class="mail-list-admin">
		...
		<form method="post" action="">
			...
			<label>Contact text</label><input maxlenght="255" type="text" name="label6" value="<?php echo get_option('mail_list_label6'); ?>">
