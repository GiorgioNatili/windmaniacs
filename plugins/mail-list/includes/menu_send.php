...
function send_the_newsletter($sender,$subject,$content){
	...
	if(count($bubi_tables)>0){
			...
			{		
				...
				//send the email
        		$headersss  = 'MIME-Version: 1.0' . PHP_EOL;
        		$headersss .= 'Content-type: text/html; charset=utf-8' . PHP_EOL;
        		$headersss .= 'From:'. $sender  . PHP_EOL;
				$sendthismail = mail($recipient,$subject,$content.$unsubscribe,$headersss);
				$mycounter=$mycounter+1;
			}			
	}
	echo '<div class="updated"><p>'.$mycounter.' newsletter sent</p></div>';
	/*echo '<div class="ERRORS"><p>'.$sendthismail.'</p></div>';*/
}
