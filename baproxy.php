...
if ( !$url ) {
  ...
  $pos = strpos($url, $base_domain);

  if ($pos === false) {
	
	$contents = 'ERROR: not allowed url';
 	$status = array( 'http_code' => 'ERROR' );
	  ...
	  if ($send_cookies && $_GET['send_cookies'] ) {
	  ...
	  $user_agent = $_GET['user_agent'] ? $_GET['user_agent'] : $_SERVER['HTTP_USER_AGENT'];
